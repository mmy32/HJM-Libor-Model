...
for i in range(n_pcs):
    ...
    axes[i, 0].axhline(
        y=params["theta"], color="r", linestyle="--", label=f"θ = {params['theta']:.3f}"
    )
    ...
    theoretical_pdf = (1 / (stationary_std * np.sqrt(2 * np.pi))) * np.exp(
        -0.5 * ((x_range - stationary_mean) / stationary_std) ** 2
    )
    axes[i, 1].plot(
        x_range,
        theoretical_pdf,
...
for i in range(n_pcs):
    ...
    print(
        f"{pc_name:<6} {params['kappa']:>10.4f}  {params['half_life_days']:>12.1f} d  "

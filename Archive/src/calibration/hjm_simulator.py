...
class HJMSimulator:
    ...
    def __init__(self, data_dir="data/ns_parameters"):
        ...
        self.pc_sensitivities = {
            pc: np.array(values) for pc, values in sens_data["pc_sensitivities"].items()
        }
        ...
        self.pc_vols = np.array([self.ou_params[pc]["sigma"] for pc in self.pc_names])
    ...
    def _pcs_to_ns_params(self, alpha):
        ...
        return {"b0": params[0], "b1": params[1], "b2": params[2], "lambda": params[3]}
    ...
    def _ns_params_to_curve(self, params):
        ...
        return self.nelson_siegel_forward(
            self.maturities, params["b0"], params["b1"], params["b2"], params["lambda"]
        )
        ...
        for step in range(1, n_steps):
            ...
            for path in range(n_paths):
                ...
                if measure == "P":
                    ...
                    d_alpha = self._evolve_pcs_Q_measure(alpha_current, dt, dW[path], lambda_risk)
                else:
    ...
    def plot_sample_paths(self, results, n_sample=5, plot_type="zero_rates"):
        ...
        for pc_idx in range(min(3, self.n_pcs)):  # Plot first 3 PCs
            for path in sample_paths:
                ax.plot(time_grid, results["pc_paths"][path, :, pc_idx], alpha=0.6, linewidth=1)
            # Add mean
        ...
        for path in sample_paths:
            if plot_type == "zero_rates":
                ax.plot(
                    maturities, results["zero_curves"][path, -1, :] * 100, alpha=0.6, linewidth=1.5
                )
            else:
                ax.plot(
                    maturities,
                    results["forward_curves"][path, -1, :] * 100,
                    alpha=0.6,
                    linewidth=1.5,
                )
        # Add mean curve
        ...
        for path in sample_paths:
            if plot_type == "zero_rates":
                ax.plot(
                    time_grid,
                    results["zero_curves"][path, :, idx_10y] * 100,
                    alpha=0.6,
                    linewidth=1,
                )
            else:
                ax.plot(
                    time_grid,
                    results["forward_curves"][path, :, idx_10y] * 100,
                    alpha=0.6,
                    linewidth=1,
                )
...
if __name__ == "__main__":
    ...
    results_P = sim.simulate(
        n_paths=1000, T_horizon=0.25, dt=1 / 252, measure="P", random_seed=42  # 3 months
    )

    # Test 2: Risk-neutral measure (pricing)
    print("\n[TEST 2] Risk-Neutral Measure (Q) - Medium horizon")
    results_Q = sim.simulate(
        n_paths=1000, T_horizon=1.0, dt=1 / 52, measure="Q", random_seed=42  # 1 year  # Weekly
    )

    # Plot results
    print("\nGenerating visualizations...")
    fig_P = sim.plot_sample_paths(results_P, n_sample=10)
    fig_P.savefig("data/ns_parameters/hjm_simulation_P_measure.png", dpi=300, bbox_inches="tight")
    plt.show()

    fig_Q = sim.plot_sample_paths(results_Q, n_sample=10)
    fig_Q.savefig("data/ns_parameters/hjm_simulation_Q_measure.png", dpi=300, bbox_inches="tight")
    plt.show()

...
def ns_sensitivities(tau_values, beta0, beta1, beta2, tau):
    ...
    return {"dbeta0": df_dbeta0, "dbeta1": df_dbeta1, "dbeta2": df_dbeta2, "dtau": df_dtau}
...
maturities = np.array([0.25, 0.5, 0.75, 1, 1.5, 2, 3, 4, 5, 6, 7, 8, 9, 10, 12, 15, 20, 25, 30])
...
for i, pc in enumerate(["PC1", "PC2", "PC3", "PC4"]):
    ...
    ax.annotate(
        f"Max impact at {max_impact:.1f}Y",
...
sensitivity_data = {
    ...
    "pc_sensitivities": {pc: pc_sens_df[pc].tolist() for pc in pc_sens_df.columns},
    "mean_parameters": {
